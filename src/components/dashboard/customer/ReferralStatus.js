    ...
    async function checkAffiliate() {
      try {
        ...
        if (response.length > 0) {
          ...
          // ✅ If not an affiliate, fetch customer details from Commerce7 API
          const headers = {
            tenant: tenantId,
            "Content-Type": "application/json",
          };

          const customerResponse = await fetchData("commerce7", `customer/${customerId}`, "GET", null, headers);

          if (customerResponse) {
            const fullName = `${customerResponse.firstName} ${customerResponse.lastName}`.trim();
            setCustomerName(fullName);
            setCustomerEmail(customerResponse.emails?.length > 0 ? customerResponse.emails[0].email : "No email available");
          }
        }
      } catch (err) {
  ...
  const createAffiliate = async () => {
    ...
    try {
      const response = await fetchData("amazon", "affiliate", "POST", {
        tenantId,
        customerId,
        name: customerName || "Unknown Customer", // ✅ Use fetched name instead of hardcoded value
      });
      ...
      <CardContent>
        ...
        {!loading && !affiliate && (
          <div>
            ...
            {/* ✅ Display customer details fetched from Commerce7 API */}
